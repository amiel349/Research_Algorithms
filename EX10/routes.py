...
@app.route('/google-input', methods=('GET', 'POST'))
def google_sheets_graph():
    ...
    nodes, edges, res = read_and_write_gspread(str(url.data))
    form = RandomGraphForm()
    sheets = GoogleSheetsForm()
    flag = True
    return render_template('google-input.html', sheets=sheets, form=form, nodes=nodes, edges=edges, res=res)
